...
namespace Kursova2.UI
{
    public class ClientForm : BaseLibraryForm
    {
        ...
        public ClientForm(ApiClient apiClient) : base(apiClient)
        {
            ...
            // Загружаем таблицы асинхронно после создания формы
            this.Load += async (s, e) => await LoadTablesAsync(isAdmin: false);
        }
        ...
        private void InitializeClientComponents()
        {
            ...
            btnMyProfile.Click += (s, e) => ShowUserDataAsync(GetUserName(), "Profile");
            ...
            btnMyFines.Click += (s, e) => ShowUserDataAsync(GetUserName(), "Fines");
            ...
            btnMyReservations.Click += (s, e) => ShowUserDataAsync(GetUserName(), "Reservations");
            ...
            btnLibraryChart.Click += async (s, e) => {
                try
                {
                ...
                catch (Exception ex) { MessageBox.Show("Помилка графіка: " + ex.Message); }
            };
            ...
            btnRegister.Click += (s, e) => new RegistrationForm(_apiClient).ShowDialog();
        ...
        private async void BtnReserve_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                string resultMessage = await _apiClient.ReserveBookAsync(bookId, userName);
                MessageBox.Show(resultMessage);

                if (comboBoxTables.SelectedItem != null) await LoadDataAsync(comboBoxTables.SelectedItem.ToString());
            }
            catch (Exception ex)
            {
        ...
        private async void BtnCancelReservation_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                string result = await _apiClient.CancelReservationAsync(reservationId);
                MessageBox.Show(result);

                ShowUserDataAsync(GetUserName(), "Reservations");
            }
            catch (Exception ex)
            {
        ...
        private async void ShowUserDataAsync(string userName, string mode)
        {
            if (userName == null) return;
            ...
            {
                DataTable data = null;
                if (mode == "Profile") data = await _apiClient.GetReaderInfoAsync(userName);
                else if (mode == "Fines") data = await _apiClient.GetUserFinesAsync(userName);
                else if (mode == "Reservations") data = await _apiClient.GetUserReservationsAsync(userName);
